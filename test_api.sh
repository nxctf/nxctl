#!/usr/bin/env bash
# Simple testing script for ctfc API
...
echo "--- Testing Root ---"
curl -s -X GET "$API_URL/" -H "X-CTFC-Token: $TOKEN" | jq .

echo -e "\n--- Testing Challenges List ---"
curl -s -X GET "$API_URL/challenges" -H "X-CTFC-Token: $TOKEN" | jq .

echo -e "\n--- Testing UP ($CHALLENGE) ---"
curl -s -X POST "$API_URL/up/$CHALLENGE" -H "X-CTFC-Token: $TOKEN" | jq .
...
curl -s -X POST "$API_URL/restart/$CHALLENGE" -H "X-CTFC-Token: $TOKEN" | jq .

echo -e "\n--- Testing Down ($CHALLENGE) ---"
curl -s -X POST "$API_URL/down/$CHALLENGE" -H "X-CTFC-Token: $TOKEN" | jq .
