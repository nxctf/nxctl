#!/usr/bin/env python3
"""CTF Challenge Orchestration Engine - Main CLI Application."""
...
# Auto-load .env into process environment
try:
...
from src.scripts.challenges import (
    cmd_challenge_sync,
    cmd_challenge_list,
...
from src.scripts.runtime import (
    cmd_runtime_build,
    cmd_runtime_start,
    cmd_runtime_stop,
    cmd_runtime_status,
...
from src.scripts.exports import (
    cmd_export_ngrok,
...
def main():
    ...
    force_stop_cmd.add_argument("name", nargs="?", default=None, help="Challenge name (optional)")
    ...
    export_ngrok.add_argument("name", help="Challenge name (e.g., web/sqli)")
    export_ngrok.set_defaults(func=cmd_export_ngrok)

    # export localtunnel
    export_lt = export_sub.add_parser("localtunnel", help="Export via localtunnel")
    export_lt.add_argument("name", help="Challenge name (e.g., web/sqli)")
    export_lt.set_defaults(func=cmd_export_localtunnel)

    # export pinggy
    export_pinggy = export_sub.add_parser("pinggy", help="Export via Pinggy TCP tunnel")
    export_pinggy.add_argument("name", help="Challenge name")
    export_pinggy.set_defaults(func=cmd_export_pinggy)
    ...
    export_list.add_argument("--all", action="store_true", help="Show all history rows")
    ...
    export_stop.add_argument("name", help="Challenge name")
    export_stop.add_argument("provider", nargs="?", choices=["ngrok", "localtunnel", "pinggy"], help="Optional provider")
    ...
    export_prune.add_argument("provider", nargs="?", choices=["ngrok", "localtunnel", "pinggy"], help="Optional provider filter")
