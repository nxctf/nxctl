...
check_dependencies() {
    ...
    if ! command -v python3 >/dev/null 2>&1; then
        ...
        case $yn in
            [Yy]*) sudo apk update && sudo apk add python3 py3-pip python3-dev build-base musl-dev libffi-dev ;;
    ...
    if ! command -v npm >/dev/null 2>&1; then
        ...
        read -p "👉 Do you want to install it now? (y/n): " yn
        case $yn in
            [Yy]*) sudo apk update && sudo apk add nodejs npm ;;
    ...
    if ! command -v docker >/dev/null 2>&1; then
        ...
        case $yn in
            [Yy]*)
                sudo apk update && sudo apk add docker
    ...
    if ! docker compose version >/dev/null 2>&1; then
        ...
        read -p "👉 Do you want to install it now? (y/n): " yn
        case $yn in
            [Yy]*) sudo apk update && sudo apk add docker-cli-compose ;;
...
enable_service() {
    ...
    sudo rc-update add ctfc-daemon default
    sudo service ctfc-daemon restart
...
disable_service() {
    ...
    sudo service ctfc-daemon stop
    sudo rc-update del ctfc-daemon default
    sudo rm -f "$INITD_TARGET"
