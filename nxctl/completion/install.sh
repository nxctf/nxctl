...
echo "Installing NXCTL bash completion..."
...
if [[ -d "/etc/bash_completion.d" ]] && [[ $EUID -eq 0 ]]; then
    cp "${COMPLETION_SCRIPT}" /etc/bash_completion.d/nxctl
    echo "Installed system-wide to /etc/bash_completion.d/nxctl"
else
    if [[ -d "${HOME}/.bash_completion.d" ]]; then
        cp "${COMPLETION_SCRIPT}" "${HOME}/.bash_completion.d/nxctl"
        echo "Installed for current user to ${HOME}/.bash_completion.d/nxctl"
    elif [[ -f "${HOME}/.bashrc" ]]; then
        sed -i "\|ctfs-back-completion.bash|d" "${HOME}/.bashrc" 2>/dev/null || true
        if ! grep -q "nxctl-completion.bash" "${HOME}/.bashrc"; then
            echo "" >> "${HOME}/.bashrc"
            echo "# NXCTL bash completion" >> "${HOME}/.bashrc"
